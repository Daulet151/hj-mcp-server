...
class DatabaseManager:
    ...
    def log_bot_user(self, user_info: dict):
        """
        Insert or update bot user information for analytics.

        Args:
            user_info: Dict with user info from Slack API
        """
        try:
            with self.get_connection() as conn:
                with conn.cursor() as cur:
                    ...
                    conn.commit()
                    logger.debug(f"Logged user: {user_info.get('slack_user_id')}")
        except Exception as e:
            logger.error(f"Failed to log bot user: {e}")
                ...
                with conn.cursor() as cur:
                    sql = """
                        INSERT INTO analytics.bot_interactions (
                            session_id, slack_user_id, slack_username, real_name,
                            channel_id, user_message, query_type, bot_response,
                            sql_query, sql_executed, sql_execution_time_ms, rows_returned,
                            table_generated, error_message
                        ) VALUES (
